...
class BountyAggregator {
  constructor() {
    ...
    this.ecosystems = [
      {
        name: 'Owockibot',
        url: 'https://www.owockibot.xyz/api/bounty-board?status=open',
      ...
      {
        name: '4Claw',
        url: 'https://www.4claw.org/api/v1/boards/job/threads',
        claimUrl: 'https://www.4claw.org',
        transform: this.transform4Claw.bind(this),
        enabled: false, // Requires API key
        headers: {
          'Authorization': `Bearer ${process.env.FOURCLAW_API_KEY || ''}`
        }
      },
      // Note: Adding placeholder for Moltboard - would need live endpoint
      {
        name: 'Mock Demo Data',
        url: null, // Mock data for demonstration
        transform: this.transformMockData.bind(this),
        enabled: true
      }
    ];
  }
  ...
  async getAllBounties(filters = {}) {
    ...
    Object.entries(this.bounties).forEach(([ecosystemName, bounties]) => {
      ...
      const filteredBounties = bounties.filter(bounty => {
        ...
        // Status filter
        if (status && bounty.status !== status) return false;
